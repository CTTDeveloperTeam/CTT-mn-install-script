...
RELEASEFILE="https://github.com/SphinxCrypto/test/releases/download/v1.0/Cryptoinvest.tar.gz"
SENTINELGIT="" # leave empty if coin has no sentinel

instance="" # only when doing multiple instances
daemon="cryptoinvestd"
cli="cryptoinvest-cli"
stopcli="stop"
archive_path="Cryptoinvest/bin/"
core_dir=".cryptoinvest"
config_path="$core_dir/cryptoinvest.conf"
node_user="cct""$instance"
mainnet="57810"
...
function install_cronjob {
	...
	crontab -l | { cat; echo "*/30 * * * * (( (\$(curl -s http://45.76.214.165/api/getblockcount) - \$($cli getblockcount)) > 10 )) && systemctl is-enabled $daemon$instance && systemctl restart $daemon$instance-reindex" ;} | crontab -
}
